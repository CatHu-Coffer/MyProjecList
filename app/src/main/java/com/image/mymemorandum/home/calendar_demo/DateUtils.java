...
public class DateUtils {
     ...
     * @return
     */
    public static List<String> getDayOfMonthFormats(int year, int month) {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 7; j++) {
                if (i == 0 && j < daysOfFirstWeek - 1) {
                    int day = daysOfLastMonth - daysOfFirstWeek + 2 + j;
                    daylist.add(String.valueOf(day));
                } else if (dayNum <= daysOfMonth) {
                    daylist.add(String.valueOf(dayNum++));
                } else {
                    daylist.add(String.valueOf(nextDayNum++));
                }
            }
        }
        calendarBean.setMonth(String.valueOf(month));
        calendarBean.setYear(String.valueOf(year));
        calendarBean.setDay(daylist);
        return daylist;
    }
